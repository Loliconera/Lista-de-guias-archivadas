...
function skilld_event(skillid, handlers, event, entity, dispatch) {
	...
	if (skillid == 120) {
		...
		// к нему (бублик вокруг босса) -> от него (круг перед боссом) -> [волны] -> к нему (бублик перед боссом)
			...
			handlers['text']({"type": "text","sub_type": "message","message": "ENTRAR > SALIR","message_RU": "К нему > От него > (К нему)"});
	...
	if (skillid == 123) {
		...
		// от него (круг перед боссом) -> к нему (бублик вокруг босса) -> [волны] -> от него (большой круг перед боссом)
			...
			handlers['text']({"type": "text","sub_type": "message","message": "SALIR > ENTRAR","message_RU": "От него > К нему > (От него)"});
			handlers['text']({"type": "text","sub_type": "message","delay": 5000,"message": "SALIR","message_RU": "От него"});
		...
		// к нему (бублик вокруг босса) -> от него (круг перед боссом) -> [волны] -> от него (большой круг перед боссом)
			...
			handlers['text']({"type": "text","sub_type": "message","message": "ENTRAR > SALIR","message_RU": "К нему > От него > (От него)"});
			handlers['text']({"type": "text","sub_type": "message","delay": 5000,"message": "SALIR","message_RU": "От него"});
	...
	if (skillid == 127) {
		if (boss_thirty) {
			handlers['text']({"type": "text","sub_type": "message","message": "Gran Salto","message_RU": "Прыжок | От него"});
		} else {
			handlers['text']({"type": "text","sub_type": "message","message": "Gran Salto","message_RU": "Прыжок | К нему"});
...
let debuffs_targe = {
	30209101: {msgt: 'Relámpago', msg: 'Молния (эвейд)'},
	30209102: {msgt: 'Bruja',     msg: 'Ведьма (эвейд)'}
};
...
module.exports = {
	...
	// 1 BOSS
	"s-3020-1900-104-0": [{"type": "text","sub_type": "message","message": "Succión (iframe)","message_RU": "Высасывание (Выйти)"},{"type": "func","func": SpawnCircle.bind(null,true,553,0,0,15,450,200,6000)}],

	// 2 BOSS
	"s-3020-1200-103-0": [{"type": "text","sub_type": "message","message": "Succión (iframe)","message_RU": "Высасывание"}],

	// 3 BOSS
	"s-3020-2200-108-0": [{"type": "text","sub_type": "message","message": "Stun frontal","message_RU": "Стан"},{"type": "func","func": SpawnCircle.bind(null,true,553,0,170,20,120,200,2000)}],
	...
	"s-3020-2200-129-0": [{"type": "text","sub_type": "message","message": "Lanzamiento de Martillo ~ Cráneo","message_RU": "Полоса в цель"},
	...
	"s-3020-2200-133-1": [{"type": "text","sub_type": "message","message": "Donas","message_RU": "Бублики"},
	...
	"s-3020-2200-135-0": [{"type": "text","sub_type": "message","message": "Charcos Inc (Saltar)","message_RU": "Волны х5"}],
	...
	"s-3020-2200-202-0": [{"type": "text","sub_type": "message","message": "Defensa 3seg.","message_RU": "Защита 3 сек."}],
	"s-3020-2200-203-0": [{"type": "text","sub_type": "message","message": "Defensa 10seg.","message_RU": "Защита 10 сек."}],
	"s-3020-2200-204-0": [{"type": "text","sub_type": "message","message": "30% transformación","message_RU": "30% видоизменение"}]
};