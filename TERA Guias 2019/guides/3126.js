...
const CK_TipMsg =
{
	...
	2: {msgt: 'Izquierda',  msg: 'Лево'},
	3: {msgt: 'Derecha', msg: 'Право'}
};
	...
	if ([213,214].includes(skillid)) {   // Ice inside
		setTimeout(() => {
			if (debuff != null) {
				handlers['text']({
					"sub_type": "message",
					"message": (`Hielo Interior (${qbacting_TipMsg[qbacting].msgt}) | ${CK_TipMsg[debuff % 2 + 2].msgt} | ${CK_TipMsg[(qbacting + debuff + 1) % 2].msgt}`),
	...
	if ([212,215].includes(skillid)) {   // Fire inside
		setTimeout(() => {
			if (debuff != null) {
				handlers['text']({
					"sub_type": "message",
					"message": (`Fuego Interior (${qbacting_TipMsg[qbacting].msgt}) | ${CK_TipMsg[debuff % 2 + 2].msgt} | ${CK_TipMsg[(qbacting + debuff) % 2].msgt}`),
...
let skills = {
	...
	"114-0": [{"type": "text","sub_type": "message","message": "Ataque de Fuego frontal","message_RU": "Огонь впереди"}],
	...
	//
	"s-3126-1000-1107-0": [{"type": "text","sub_type": "message","message": "[Debuff] Más lejano","message_RU": "[Дебафф] Дальние"}],
	"s-3126-1000-2107-0": [{"type": "text","sub_type": "message","message": "[Debuff] Más cercano","message_RU": "[Дебафф] Ближние"}],
	"am-3126-1000-31260251": [{"type": "text","sub_type": "message","message": "[Debuff] Capa 1","message_RU": "[Дебафф] 1 стак"}],
	"am-3126-1000-31260067": [{"type": "text","sub_type": "message","message": "[Debuff] Capa 2","message_RU": "[Дебафф] 2 стак"}],
	"am-3126-1000-31260068": [{"type": "text","sub_type": "message","message": "[Debuff] Capa 3","message_RU": "[Дебафф] 3 стак"},
		{"type": "text","sub_type": "message","delay": 145000,"message": '[Debuff] 2.5 minutos pasaron',"message_RU": "[Дебафф] Прошло 2.5 минуты (стаки удалены)"}
	],
};
